...
var createAndSavePerson = function(done) {
  ...
  person.name = "Ivan";
  person.age = 30;
  person.favoriteFoods = ["revuelto de gramajo", "hamburguesa", "pasta"];
...
var createManyPeople = function(arrayOfPeople, done) {
  Person.create(arrayOfPeople, (err, array) => {
    if (err) return console.error(err);
    done(null, array);
  });
};
...
var findEditThenSave = function(personId, done) {
  ...
  findPersonById(personId, (e, personFound) => {
    if (e)
    {
      console.error(e);
      done(e);
    }
    personFound.favoriteFoods = [...personFound.favoriteFoods, foodToAdd];
    
    personFound.save((error, personSaved) => {
      if (e)
      {
        console.error(e);
        done(e);
      }
      done(null, personSaved);
    });
  });
};
